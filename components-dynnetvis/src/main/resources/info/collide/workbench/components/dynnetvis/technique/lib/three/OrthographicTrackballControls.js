...
THREE.OrthographicTrackballControls = function ( object, domElement ) {
	...
	var _this = this;
	/**
	 * modified - changed right / left click
	...
	// methods
	/**
	 * NEW ( controls could not handle changed camera resolution )
	 * @author henrik detjen
	 */
	this.setCamera = function (cam){
		
		// reset borders 
		this.position0 = cam.position.clone();
		this.up0 = cam.up.clone();
	...
	/**
	 * modifed - now relative to domElement
	 * @author henrik detjen
	 */
	this.handleResize = function (camera) {

		// do this relative to the object not window...
		this.screen.width = domElement.offsetWidth;//window.innerWidth;
		this.screen.height = domElement.offsetHeight;//window.innerHeight;
		
		this.screen.offsetLeft = 0;
		this.screen.offsetTop = 0;
		this.radius = ( this.screen.width + this.screen.height ) / 4;			
		
		if (camera != null) this.setCamera(camera);
	...
	/**
	 * modified - forceUpdate
	 * @author henrik detjen
	 */
	this.zoomCamera = function ( forceUpdate ) {
		...
		if ( _state === STATE.TOUCH_ZOOM ) {
			...
			if ( factor !== 1.0 && factor > 0.0 || forceUpdate == true ) {
				_zoomFactor *= factor;
