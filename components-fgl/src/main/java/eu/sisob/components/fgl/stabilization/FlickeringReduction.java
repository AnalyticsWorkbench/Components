...
public class FlickeringReduction extends DefaultStabilization{
	
	private Vector<Integer> flickeringMap;
	
	public FlickeringReduction(){
		super();
		flickeringMap = new Vector<Integer>();
	}
	...
	private Node requestFlickeringNode(NodeSet currentEntities) {
		...
		for(int i=0;i<flickeringMap.size();i++){
			if(flickeringMap.get(i)>maxFlickering){
				foundIndex = i;
				priorityNode = (Node)currentEntities.get(i);						
			}
		}
		
		flickeringMap.remove(foundIndex);
	...
	private void buildPriorityMap(NodeSet currentEntities) {
		...
		for(int i=0;i<currentEntities.size();i++){
			int flicker = calculateFlicker((Node)currentEntities.get(i));
			flickeringMap.add(flicker);
		}
		
	}
	
	private int calculateFlicker(Node currentEntity) {
		// TODO Auto-generated method stub		
		int row = super.getEntitiesOverTime().indexOf(currentEntity.getLabel());
		int flicker = 0;
		for(int column=0;column<super.getNetworks().size();column++ ){
			if(column+1<super.getNetworks().size() && (super.getTimeMatrix()[row][column]!=super.getTimeMatrix()[row][column+1])){
				flicker++;
			}			
		}	
		return flicker;
