...
import eu.sisob.api.visualization.technique.interfaces.ClientSideCalculation;
import eu.sisob.api.visualization.technique.interfaces.MetadataUpdate;
import eu.sisob.api.visualization.technique.interfaces.NodeSetUpdate;
import eu.sisob.components.framework.json.util.IllegalContentTypeException;
import eu.sisob.components.framework.json.util.JSONFile;


public class SwimLaneTechnique extends VisualizationTechnique implements ClientSideCalculation, NodeSetUpdate, MetadataUpdate{
			...
			if(xAxisCriteria.equalsIgnoreCase(SwimLaneTechnique.DEFAULT_X_CRITERIA)){
				...
				Iterator<SwimLaneNode>nodeIterator = network.getVertices().iterator();
	    		 while(nodeIterator.hasNext()){
	    			 ...
	    			 else throw new IllegalContentTypeException("The requested criteria is not available in the data set.");	    			 
	    		 }
			}
		}else throw new IllegalContentTypeException("SwimLane Error: Unable to generate axis with empty criteria.");
	...
	public Metadata updatedMetadata() throws Exception {
		...
		if(properties!=null){
			...
			p.setPropertyKey("xvalue");
			p.setParsingType("string");
			properties.addProperty(p);
			p = new Property();
			p.setPropertyKey("yvalue");
			p.setParsingType("string");
			properties.addProperty(p);
