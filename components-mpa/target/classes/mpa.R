# functions that identify the main path(s) in a given graph
#
# Author: hecking, steinert
###############################################################################

#source("edge_weightening_schemes.R")
...
sapply(labels, function(l) {
    ...
    cat("Checking whether graph is a dag: ")
    cat(is.dag(graph))
    cat("\n")
    
    if(!is.dag(graph)){
    	cat("Error: Graph is not a DAG!\n")
    	stop("Graph is not a DAG!"); 
    }
    
    g <- mpa(graph, ews, th=threshold)   
    write.graph(g, paste(l,"_mpdec.gml",sep=""), "gml")
    
    sg <- subgraph.edges(g, which(E(g)$mp == "TRUE"))
    write.graph(sg, paste(l,"_mp.gml",sep=""), "gml")

    if (isFirst) {
        isFirst <<- FALSE
        decUrls <<- paste(l,"_mpdec.gml",sep="")
        resUrls <<- paste(l,"_mp.gml",sep="")
    } else {    
        decUrls <<- paste(decUrls,",",l,"_mpdec.gml",sep="")
        resUrls <<- paste(resUrls,",",l,"_mp.gml",sep="")        
    }
