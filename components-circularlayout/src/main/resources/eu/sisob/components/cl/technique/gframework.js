...
function VisualizationFramework(params) {
	...
	function initNodeAttibutes() { // called in initialize()
		...
		nodeAttributes = nodeMeasures.slice();
		for ( var i = 0; i < nodeproperties.length; i++) {
			var nodeProperty = nodeproperties[i];
			var propertyName = Object.keys(nodeProperty)[0];
			if (!propertyAlreadyInMeasure(propertyName, nodeMeasures)) {
				var t = "string";;
				if ( typeof(nodeProperty[propertyName]) != "string" ){
					t = nodeProperty[propertyName][0];
				}else{
					t = nodeProperty[propertyName];
				}
				var attribute = {
					title : propertyName,
					description : "none",
	...
	function initEdgeAttibutes() { // called in initialize()
		...
		edgeAttributes = edgeMeasures.slice();
		for ( var i = 0; i < edgeproperties.length; i++) {
			var edgeProperty = edgeproperties[i];
			var propertyName = Object.keys(edgeProperty)[0];
			if (!propertyAlreadyInMeasure(propertyName, edgeMeasures)) {
				var t = "string";
				if ( typeof(edgeProperty[propertyName]) != "string" ){
					t = edgeProperty[propertyName][0];
				}else{
					t = edgeProperty[propertyName];
				}
				var attribute = {
					title : propertyName,
					description : "none",
	...
	// helper s.a.
	function propertyAlreadyInMeasure(name, measures) {
		for ( var i = 0; i < measures.length; i++) {
			if (name == measures[i].property) {
				return true;
			}
		}
		return false;
