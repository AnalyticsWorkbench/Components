...
public class CSVParser extends DataTableParser {
	...
	public void parse() throws IllegalContentTypeException {
		...
		try {
			...
			if (headers.size() == measures.size()) {
				...
				// Measures networkMeasures = new Measures();

				for (int i = 0; i < headers.size(); i++) {
					String key = headers.get(i);
					Boolean isValueAMeasure = measures.get(i);
					if (isValueAMeasure) {
						// Measure m = new Measure(key, key.toLowerCase(),
						// "number", "node", "");
						// networkMeasures.addMeasure(m);
						nodeProperties.addProperty(new Property(key, "double"));
					} else {
						// Property p = new Property(key);
						// p.addValue("string");
						// nodeProperties.addProperty(p);
						nodeProperties.addProperty(new Property(key));
					}
				}

				// metadata.setNetworkMeasures(networkMeasures);
			...
			for (int i = 0; i < keys.size(); i++) {
				if ((i + 1) < keys.size()) {
					if (keys.get(i).equals("label")) {
						encodedCSV.append("\"" + jobject.get(keys.get(i)) + "\"" + ",");
					} else {
						encodedCSV.append(jobject.get(keys.get(i)) + ",");
					}
