...
#componentSelector <- 'SumOfEdgeWeights'
componentSelector <- 'NumberOfNodes'
#thetaSelector <- 'FixedNumber'
thetaSelector <- 'SeedIsLost'
#thetaSelector <- 'Intersection'
#edgeWeightName <- 'linguisticSimilarity'
#edgeWeightName <- 'topicSimilarity'
#edgeWeightName <- 'topicSimilarity'
...
g <- read.graph('C:/Users/steinert/eigene Forschung/Weighted_Citation_Networks/SVN/Versuche/Prediction_Evaluation/Hulth/1_mpdec.gml','gml')
...
sapply(labels, function(l) {
  graph <- graphs[[l]]
	...
	sink("C:/Users/steinert/eigene Forschung/Weighted_Citation_Networks/SVN/Versuche/Prediction_Evaluation/Hulth/output.csv", append=FALSE, split=TRUE)
...
#				cat("\n")
    ...
    # write decorated graph
   	write.graph(graph, paste(outputPath, graphname, ".gml", sep=""), "gml")
    decUrls <<- paste(graphname, ".gml", sep="")
    
    # write result
    # sometimes the filename is too long which causes an error "cannot write edgelist..."
    filename <- paste(outputPath, graphname, "_biggestComponent.gml", sep="")   
    write.graph(largestComponents[[index]], filename, "gml")

  for(v in which(V(graph)$isInNodeSubset == "TRUE")){
    label = get.vertex.attribute(graph, "label", index=v)
    print(label)
  }
